...
class Sentence():
    ...
    def known_mines(self):
        ...
        """
        # If quantity of sells in a set is equal to count, then all cells are mines.
    ...
    def known_safes(self):
        ...
        """
        # If count is equal to 0, then cells are safe.
...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        i, j = cell
        adjacent_cells = [(x, y) for x in range(max(0, i - 1), min(self.height, i + 2)) for y in
    ...
    def make_safe_move(self):
        ...
        """
        if self.safes != set():
            # Filter the list of safe movies to leave only movies that have not already been chosen.
            safe_moves = [m for m in list(self.safes) if m not in self.moves_made]
            # Return randomly chosen move from previously filtered list if it is not empty.
            return random.choice(safe_moves) if any(safe_moves) else None
        return None
    ...
    def make_random_move(self):
        ...
        """
        # Get list of all possible movies.
        all_moves = [(i, j) for i in range(self.height) for j in range(self.width)]
        # Filter the list to leave only movies that have not already been chosen and not mines.
        safes = [m for m in all_moves if m not in self.moves_made.union(self.mines)]
        # Return randomly chosen move from previously filtered list if it is not empty.
        return random.choice(safes) if any(safes) else None
